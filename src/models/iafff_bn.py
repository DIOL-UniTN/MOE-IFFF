...
class IAFFFBN(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, out_features: int, depth: int):
        ...
        self.register_buffer('alpha', torch.tensor(3.0))  # Registered buffer
        ...
        self.bn_weights = nn.Parameter(torch.ones((self.n_nodes, 1)), requires_grad=True)
        self.bn_biases = nn.Parameter(torch.zeros((self.n_nodes, 1)), requires_grad=True)
        self.register_buffer(
            "running_mean", torch.zeros(self.n_nodes, 1)
        )
        self.register_buffer(
            "running_var", torch.ones(self.n_nodes, 1)
        )
        self.eps = 1e-05
        self.momentum = 0.1
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth):
            ...
            # BN
            current_bn_weights = self.bn_weights[platform:next_platform] # (n_nodes, in_features)    
            current_bn_biases = self.bn_biases[platform:next_platform]   # (n_nodes, 1)
            current_running_mean = self.running_mean[platform:next_platform] # (n_nodes, in_features)    
            current_running_var = self.running_var[platform:next_platform] # (n_nodes, in_features)    

            # Matmul forward
            scores = torch.matmul(x, current_weights.transpose(0, 1))      # (batch_size, n_nodes)
            scores = scores + current_biases.transpose(0, 1)# (batch_size, n_nodes)
            # BN forward
            mean, var = scores.mean(dim=0, keepdim=True), scores.var(dim=0, unbiased=False, keepdim=True)  # [n_nodes, features], [n_nodes, features]
            scores = (scores - mean) / torch.sqrt(var + self.eps)
            scores = current_bn_weights.transpose(0, 1) * scores + current_bn_biases.transpose(0, 1)
            # BN update running stats
            self.running_mean[platform:next_platform] = (1 - self.momentum) * current_running_mean + self.momentum * mean.transpose(0, 1)
            self.running_var[platform:next_platform] = (1 - self.momentum) * current_running_var + self.momentum * var.transpose(0, 1)

            prob = torch.sigmoid(scores)                              # (batch_size, n_nodes)
    ...
    def eval_forward(self, x: torch.Tensor, leaf_ids: bool = False) -> torch.Tensor:
        ...
        for i in range(self.depth):
            # Matmul
            plane_weights = self.node_weights.index_select(dim=0, index=current_nodes)       # (batch_size, in_features)
            plane_biases = self.node_biases.index_select(dim=0, index=current_nodes)       # (batch_size, 1)
            # BN
            bn_weights = self.bn_weights.index_select(dim=0, index=current_nodes)       # (batch_size, in_features)
            bn_biases = self.bn_biases.index_select(dim=0, index=current_nodes)       # (batch_size, 1)
            running_means = self.running_mean.index_select(dim=0, index=current_nodes)       # (batch_size, in_features)
            running_vars = self.running_var.index_select(dim=0, index=current_nodes)       # (batch_size, 1)

            # Matmul forward
            scores = torch.bmm(x.unsqueeze(1), plane_weights.unsqueeze(-1))       # (batch_size, 1, 1)
            scores = scores.squeeze(-1) + plane_biases                     # (batch_size, 1)
            # BN forward
            scores = (scores - running_means) / torch.sqrt(running_vars + self.eps)
            scores = bn_weights * scores + bn_biases

            plane_choices = (scores.squeeze(-1) >= 0).long()                           # (batch_size,)
...
class IAFFFBNv2(nn.Module):
    def __init__(self, in_features: int, leaf_width: int, out_features: int, depth: int):
        ...
        self.bn_biases = nn.Parameter(torch.zeros((self.n_nodes, 1)), requires_grad=True)
        self.register_buffer(
            "running_mean", torch.zeros(self.n_nodes, self.in_features)
        )
        self.register_buffer(
            "running_var", torch.ones(self.n_nodes, self.in_features)
        )
        self.eps = 1e-05
        self.momentum = 0.1
    ...
    def training_forward(self, x: torch.Tensor):
        ...
        for current_depth in range(self.depth):
            ...
            # Matmul forward
            scores = (x[:, None, :] * current_weights[None, :])
            # BN forward
            mean, var = scores.mean(dim=0), scores.var(dim=0, unbiased=False)  # [n_nodes, features], [n_nodes, features]
            scores = (scores - mean[None, :]) / torch.sqrt(var[None, :] + self.eps)
            scores = current_bn_weights * scores + current_bn_biases
            # BN update running stats
            self.running_mean[platform:next_platform] = (1 - self.momentum) * current_running_mean + self.momentum * mean
            self.running_var[platform:next_platform] = (1 - self.momentum) * current_running_var + self.momentum * var

            routers_out = scores.sum(dim=2)
            prob = torch.sigmoid(routers_out)                              # (batch_size, n_nodes)
            not_prob = 1 - prob                                   # (batch_size, n_nodes)
    ...
    def eval_forward(self, x: torch.Tensor, leaf_ids: bool = False) -> torch.Tensor:
        ...
        for i in range(self.depth):
            ...
            # Matmul forward
            scores = (x * plane_weights)         # (batch_size, 1)
            # BN forward
            scores = (scores - running_means) / torch.sqrt(running_vars + self.eps)
            scores = bn_weights * scores + bn_biases

            routers_out = scores.sum(dim=1)
            plane_choices = (routers_out.squeeze(-1) >= 0).long()                           # (batch_size,)
