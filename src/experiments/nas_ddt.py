...
from utils.moo import NASDDTProblem, NASDDTProblemFixLeaf
from utils.moo import DDTMetricsLogger, DDTDisplay
from utils.moo import DDTRandomBalancedSampling, DDTRandomImbalancedSampling
...
SAMPLINGS = {
        "imbalanced": DDTRandomImbalancedSampling,
        "balanced": DDTRandomBalancedSampling,
        }
class NASDDT(BaseExp):
    def __init__(self, n_gens: int, pop_size: int, max_params: int, min_params: int, 
                 crossover, mutation, tree: Tree, fix_leaf: bool, sampling: str, obj: str):
        super().__init__()  # Initialize BaseExp
        self.exp_name = f"NASDDT"
        tmp_tree = tree()
        self.fix_leaf = fix_leaf
        self.obj = obj
        self.sampling = sampling # Sampling type
        self.max_depth = tmp_tree.max_depth
        self.max_width = tmp_tree.max_width
        self.max_nodes = tmp_tree.max_nodes
    ...
    def get_config(self):
        ...
        return (exp_config | self.mlflow.get_config() | self.loader.get_config())

    def setup(self, partial_model, optim, loader, a_sched, epochs: int, device, 
              mconf: object, proj_name: str, seed: int, run_name: Optional[str] = None):
        # MLFlow setup
        ...
        self.seed = seed
        self.loader = loader
        self.n_var = self.max_nodes+1 if self.fix_leaf else self.max_nodes
        self.n_obj = 2 # acc and (params | macs)
        self.n_constr = 2 # max n. params

        # Problem setup
        self.callback = DDTMetricsLogger()
        problem = NASDDTProblemFixLeaf if self.fix_leaf else NASDDTProblem 
        self.problem = problem(partial_model, optim, loader, a_sched, device, 
                               self.n_obj, n_constr=2, epochs=epochs, 
                               max_params=self.max_params, min_params=self.min_params, 
                               tree=self.tree, obj=self.obj)
    ...
    def log_exp(self, res):
        ...
        # Scatter plot
        plot = Scatter(title=f"TASK: {self.loader.name}, MAX PARAMS: {self.max_params}, MAX DEPTH: {self.max_depth}, MAX WIDTH: {self.max_width}, POP SIZE: {self.pop_size}, N GENS: {self.n_gens}, N OBJ: {self.n_obj}", 
        ...
        pareto = pareto[sorted_pareto_indices][:, [1, 0]]
        plot.add(pareto, plot_type="line", color="red", alpha=0.7, label="Pareto Front")
        # Save
        plot.save(self.out_dir/"scatter.png")
        mlflow.log_artifact(self.out_dir/"scatter.png")
    ...
    def run_exp(self):
        res = minimize(self.problem, self.algorithm, seed=self.seed, 
                       ...
                       # display=DDTDisplay(), callback=self.callback)
        return res
    ...
    def run(self, cfg):
        ...
        self.setup(cfg.model, cfg.optim, cfg.loader, cfg.a_sched, cfg.epochs, cfg.device, 
                   cfg.mlflow, cfg.proj_name, cfg.seed, run_name=None)
